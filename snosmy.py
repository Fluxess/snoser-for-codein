...
ascii_banner = pyfiglet.figlet_format("DragovKill")
...
def send_complaint(username, telegram_id, number, email, repeats, complaint_choice, proxies=None):
    ...
    if complaint_choice == "1":
        ...
        text = f"Здравствуйте, я утерял свой телеграм-аккаунт путем взлома. Я попался на ******овую ссылку, и теперь на моем аккаунте сидит какой-то человек. Он установил облачный пароль, так что я не могу зайти в свой аккаунт и прошу о помощи. Мой юзернейм — {username}, а мой айди, если злоумышленник поменял юзернейм —  {telegram_id} . Пожалуйста, перезагрузите сессии или удалите этот аккаунт, так как у меня там очень много важных данных."
    ...
    elif complaint_choice == "9":
        text = f"Доброго времени суток, листая канал я обнаружил личную информацию обо мне, прошу принять меры и удалить этот канал, это не первый случай: {username}, id aдмина {telegram_id}. Большая просьба удалить этот канал, т.к это нарушает политику сервиса telegram."
...
def complaint():
    ...
    complaint_choice = input(colored("кодеин хуесос нажми на 1 если мать шлюха или на 5 если хочешь снести сесси", "magenta"))

    if complaint_choice in ["1","5"]:
